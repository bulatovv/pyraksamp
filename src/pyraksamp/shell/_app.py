...
class SampShellApp(App):
    ...
    async def _do_connect(self) -> None:
        try:
            ...
            await self._event_log.append_line(f'Connection failed: {exc}', style='error')
            self.exit(1)
    ...
    async def _route(self, tag: str, event: tuple) -> None:
        if tag == 'connect':
            ...
            if time.monotonic() - self._connected_at > 10.0:
                await self._event_log.append_line(f'← {name} ({evt.player_id})', style='quit')
        ...
        if is_tui_command:
            ...
            if cmd is None:
                ...
                try:
                    ...
                    await self._event_log.append_line(f'Command error: {exc}', style='error')
