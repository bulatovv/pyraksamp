...
static ENGLISH_FREQ: [u32; 256] = [
    ...
    0, 3611, 845, 1077, 1884, 5870, 841, 1057, 2501, 3212, 164, 531, 2019, 1330, 3056,
    4037, // 96-111
...
fn huffman_build() -> HuffTree {
    ...
    for i in 0..256usize {
        ...
        pool[i].weight = if ENGLISH_FREQ[i] != 0 {
            ENGLISH_FREQ[i]
        } else {
            1
        };
    ...
    while sorted.len() > 1 {
        ...
        pool[parent as usize].weight = pool[lesser as usize].weight + pool[greater as usize].weight;
...
mod tests {
    ...
    const H_AAABBBCCC: &[u8] = &[
        0x95, 0x15, 0x15, 0x15, 0x2A, 0x15, 0x0A, 0x84, 0xED, 0x9D, 0xB3, 0xB4,
    ];
    const H_ABCDEFGHIJ: &[u8] = &[0x8D, 0x98, 0xFA, 0x1C, 0x0E, 0x9B, 0x09, 0x57];
    const H_QUICK: &[u8] = &[
        0xA2, 0x2A, 0x58, 0x1C, 0xCC, 0x5A, 0x50, 0xFF, 0x9F, 0x98, 0xA4, 0xB9, 0xD1, 0x53, 0xF5,
        0x75, 0xE8, 0xA6, 0xC0,
    ];
    ...
    #[test]
    fn eight_bools_occupy_one_byte() {
        let mut bs = BitStream::new();
        for _ in 0..8 {
            bs.write_bool(true);
        }
    ...
    fn alternating_bools() {
        ...
        let mut bs2 = BitStream::from_bytes(&raw);
        for i in 0..8u8 {
            assert_eq!(bs2.read_bool().unwrap(), i % 2 == 0);
        }
    }
    ...
    fn u8_roundtrip() {
        for v in [0u8, 1, 127, 128, 255] {
            ...
            bs.write_uint8(v);
            assert_eq!(
                BitStream::from_bytes(bs.as_bytes()).read_uint8().unwrap(),
                v
            );
        }
    }
    ...
    fn u16_roundtrip() {
        for v in [0u16, 1, 256, 0x1234, 0xFFFF] {
            let mut bs = BitStream::new();
            bs.write_uint16_le(v);
            assert_eq!(
                BitStream::from_bytes(bs.as_bytes())
                    .read_uint16_le()
                    .unwrap(),
                v
            );
    ...
    fn float_nan_roundtrip() {
        let mut bs = BitStream::new();
        bs.write_float_le(f32::NAN);
        assert!(BitStream::from_bytes(bs.as_bytes())
            .read_float_le()
            .unwrap()
            .is_nan());
    }
    ...
    #[test]
    fn write_read_bits_4bit_right_aligned() {
        let mut bs = BitStream::new();
        bs.write_bits(&[0b1010], 4, true);
        let raw = bs.as_bytes().to_vec();
        let mut bs2 = BitStream::from_bytes(&raw);
        let mut out = [0u8];
        bs2.read_bits(&mut out, 4, true).unwrap();
        assert_eq!(out[0], 0b1010);
    }
    #[test]
    fn write_bits_8bit_same_as_write_u8() {
        let mut a = BitStream::new();
        a.write_bits(&[0xAB], 8, true);
        let mut b = BitStream::new();
        b.write_uint8(0xAB);
        assert_eq!(a.as_bytes(), b.as_bytes());
    }
    #[test]
    fn write_bits_3_and_5_cross_byte() {
        let mut bs = BitStream::new();
        ...
        let mut bs2 = BitStream::from_bytes(&raw);
        let mut a = [0u8];
        bs2.read_bits(&mut a, 3, true).unwrap();
        let mut b = [0u8];
        bs2.read_bits(&mut b, 5, true).unwrap();
        assert_eq!((a[0], b[0]), (0b111, 0b10101));
    }
    #[test]
    fn write_bits_single() {
        let mut bs = BitStream::new();
        bs.write_bits(&[1], 1, true);
        let raw = bs.as_bytes().to_vec();
        let mut out = [0u8];
        BitStream::from_bytes(&raw)
            .read_bits(&mut out, 1, true)
            .unwrap();
        assert_eq!(out[0], 1);
    }
    #[test]
    fn write_bits_32() {
        ...
        let mut bs = BitStream::new();
        bs.write_bits(&b, 32, true);
        let raw = bs.as_bytes().to_vec();
        let mut out = [0u8; 4];
        BitStream::from_bytes(&raw)
            .read_bits(&mut out, 32, true)
            .unwrap();
    ...
    #[test]
    fn compressed_u16_branch1_nibble_6bits() {
        for v in [0u16, 1, 15] {
            let mut bs = BitStream::new();
            bs.write_compressed_uint16(v);
            assert_eq!(bs.num_bits(), 6, "v={v}");
            assert_eq!(
                BitStream::from_bytes(bs.as_bytes())
                    .read_compressed_uint16()
                    .unwrap(),
                v
            );
        }
    ...
    #[test]
    fn compressed_u16_branch2_byte_10bits() {
    ...
    #[test]
    fn compressed_u16_branch3_full_17bits() {
    ...
    #[test]
    fn compressed_u16_sizes_ascending() {
        let bits = |v: u16| {
            let mut bs = BitStream::new();
            bs.write_compressed_uint16(v);
            bs.num_bits()
    ...
    #[test]
    fn compressed_u32_nibble_smaller_than_byte() {
        let bits = |v: u32| {
            let mut bs = BitStream::new();
            bs.write_compressed_uint32(v);
            bs.num_bits()
    ...
    #[test]
    fn aligned_bytes_after_bit_writes() {
        let mut bs = BitStream::new();
        bs.write_bool(true);
        ...
        assert!(bs2.read_bool().unwrap());
        let mut out = [0u8; 2];
        bs2.read_aligned_bytes(&mut out).unwrap();
        assert_eq!(out, [0xAB, 0xCD]);
    }
    #[test]
    fn aligned_bytes_zero_length_no_write() {
        let mut bs = BitStream::new();
        bs.write_aligned_bytes(&[]);
        assert_eq!(bs.num_bits(), 0);
    }
    #[test]
    fn aligned_bytes_100_roundtrip() {
    ...
    fn skip_past_byte_boundary() {
        ...
        bs2.skip_bits(5);
        let mut out = [0u8];
        bs2.read_bits(&mut out, 3, true).unwrap();
        assert_eq!(out[0], 0b111); // top 3 bits of 0xFF
    }
    #[test]
    fn bits_remaining_decreases_on_skip() {
        let mut bs = BitStream::new();
        bs.write_uint8(0xFF);
        let raw = bs.as_bytes().to_vec();
        let mut bs2 = BitStream::from_bytes(&raw);
    ...
    #[test]
    fn compressed_string_empty() {
        assert_eq!(
            BitStream::from_bytes(H_EMPTY)
                .read_compressed_string(256)
                .unwrap(),
            ""
        );
    }
    #[test]
    fn compressed_string_a() {
        assert_eq!(
            BitStream::from_bytes(H_A)
                .read_compressed_string(256)
                .unwrap(),
            "a"
        );
    ...
    #[test]
    fn compressed_string_hello() {
        assert_eq!(
            BitStream::from_bytes(H_HELLO)
                .read_compressed_string(256)
                .unwrap(),
            "hello"
        );
    ...
    #[test]
    fn compressed_string_space() {
        assert_eq!(
            BitStream::from_bytes(H_SPACE)
                .read_compressed_string(256)
                .unwrap(),
            " "
        );
    ...
    #[test]
    fn compressed_string_longer() {
        assert_eq!(
            BitStream::from_bytes(H_QUICK)
                .read_compressed_string(256)
                .unwrap(),
    ...
    #[test]
    fn compressed_string_newline() {
        assert_eq!(
            BitStream::from_bytes(H_NEWLINE)
                .read_compressed_string(256)
                .unwrap(),
            "\n"
        );
    ...
    #[test]
    fn compressed_string_max_chars_limit() {
        // max_chars=4 → at most 3 chars (max_chars - 1)
        assert_eq!(
            BitStream::from_bytes(H_HELLO)
                .read_compressed_string(4)
                .unwrap(),
            "hel"
        );
    }
    #[test]
    fn compressed_string_underflow_no_crash() {
    ...
    #[test]
    fn bits_remaining_tracks_reads() {
        let mut bs = BitStream::new();
        bs.write_uint8(0xFF);
        let raw = bs.as_bytes().to_vec();
        let mut bs2 = BitStream::from_bytes(&raw);
    ...
    #[test]
    fn as_bytes_contains_written_data() {
        let mut bs = BitStream::new();
        bs.write_uint8(0xAB);
        bs.write_uint8(0xCD);
    ...
    #[test]
    fn float_negative_zero_roundtrip() {
        let v = -0.0f32;
        let mut bs = BitStream::new();
        bs.write_float_le(v);
        let r = BitStream::from_bytes(bs.as_bytes())
            .read_float_le()
            .unwrap();
        assert_eq!(r.to_bits(), v.to_bits());
    }
    #[test]
    fn float_one_point_five_and_large() {
    ...
    #[test]
    fn write_bits_mixed_bool_u8_bits() {
        let mut bs = BitStream::new();
        bs.write_bool(true);
        bs.write_uint8(0xCC);
        ...
        let mut out = [0u8];
        bs2.read_bits(&mut out, 2, true).unwrap();
    ...
    #[test]
    fn compressed_u16_all_key_values() {
    ...
    #[test]
    fn compressed_u16_sample_roundtrip() {
        let mut x = 0xABCDu32;
        for _ in 0..50 {
    ...
    #[test]
    fn compressed_u32_all_branches() {
        for v in [
            0u32,
            1,
            15,
    ...
    #[test]
    fn compressed_u32_sample_roundtrip() {
        let mut x = 0xDEADBEEFu32;
        for _ in 0..50 {
            x = x.wrapping_mul(1664525).wrapping_add(1013904223);
            let mut bs = BitStream::new();
            bs.write_compressed_uint32(x);
            assert_eq!(
                BitStream::from_bytes(bs.as_bytes())
                    .read_compressed_uint32()
                    .unwrap(),
    ...
    #[test]
    fn aligned_read_after_partial_bits() {
        let mut bs = BitStream::new();
        for _ in 0..5 {
            bs.write_bool(true);
        }
    ...
    #[test]
    fn compressed_string_ascii_printable_roundtrips() {
        assert_eq!(
            BitStream::from_bytes(H_HELLO_W)
                .read_compressed_string(256)
                .unwrap(),
            "Hello World"
        );
        assert_eq!(
            BitStream::from_bytes(H_TEST123)
                .read_compressed_string(256)
                .unwrap(),
            "test123"
        );
        assert_eq!(
            BitStream::from_bytes(H_AAABBBCCC)
                .read_compressed_string(256)
                .unwrap(),
            "AAABBBCCC"
        );
        assert_eq!(
            BitStream::from_bytes(H_ABCDEFGHIJ)
                .read_compressed_string(256)
                .unwrap(),
            "abcdefghij"
        );
