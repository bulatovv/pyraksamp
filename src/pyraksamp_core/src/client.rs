...
mod tests {
    ...
    #[test]
    fn display_host_resolution() {
        assert_eq!(ConnectError::HostResolution.to_string(), "could not resolve server host");
    }

    #[test]
    fn display_handshake_timeout() {
        assert_eq!(ConnectError::HandshakeTimeout.to_string(), "connection handshake timed out");
    }

    #[test]
    fn display_connection_timeout() {
        assert_eq!(ConnectError::ConnectionTimeout.to_string(), "connection request timed out");
    }

    #[test]
    fn display_rejected() {
        assert_eq!(ConnectError::Rejected.to_string(), "connection attempt failed");
    }

    #[test]
    fn display_banned() {
        assert_eq!(ConnectError::Banned.to_string(), "banned from server");
    }

    #[test]
    fn display_invalid_password() {
        assert_eq!(ConnectError::InvalidPassword.to_string(), "invalid server password");
    }

    #[test]
    fn display_no_free_slots() {
        assert_eq!(ConnectError::NoFreeSlots.to_string(), "server is full");
    }

    #[test]
    fn display_socket_bind_includes_io_message() {
        let e = io::Error::new(io::ErrorKind::AddrInUse, "address already in use");
        let msg = ConnectError::SocketBind(e).to_string();
        assert!(msg.starts_with("failed to bind UDP socket:"), "got: {msg}");
        assert!(msg.contains("address already in use"), "got: {msg}");
    }

    #[test]
    fn display_proxy_includes_io_message() {
        let e = io::Error::new(io::ErrorKind::ConnectionRefused, "connection refused");
        let msg = ConnectError::Proxy(e).to_string();
        assert!(msg.starts_with("SOCKS5 proxy error:"), "got: {msg}");
        assert!(msg.contains("connection refused"), "got: {msg}");
    }

    #[test]
    fn connect_error_implements_std_error() {
        // Compile-time proof that ConnectError satisfies std::error::Error.
        fn assert_error<E: std::error::Error>(_: E) {}
        assert_error(ConnectError::Rejected);
        assert_error(ConnectError::SocketBind(io::Error::new(io::ErrorKind::Other, "x")));
    }
}
