...
impl PySAMPClient {
    ...
    fn new(
        ...
        let proxy = proxy_host.map(|h| socks5::ProxyConfig {
            ...
            auth: proxy_username.map(|u| (u.to_string(), proxy_password.unwrap_or("").to_string())),
        });
        PySAMPClient {
        ...
        if let Err(e) = py.allow_threads(move || inner.connect(timeout)) {
            return Err(match e {
                ...
                ConnectError::InvalidPassword => {
                    SAMPInvalidPassword::new_err("invalid server password")
                }
                ...
                ConnectError::HandshakeTimeout => {
                    SAMPHandshakeTimeout::new_err("connection handshake timed out")
                }
                ConnectError::ConnectionTimeout => {
                    SAMPConnectionTimeout::new_err("connection request timed out")
                }
                ConnectError::HostResolution => {
                    SAMPHostResolutionError::new_err("could not resolve server host")
                }
    ...
    fn send_rpc(&self, rpc_id: u8, data: Option<Vec<u8>>, reliability: u8) {
        self.inner
            .send_rpc(rpc_id, &data.unwrap_or_default(), reliability);
    }
    ...
    fn send_dialog_response(
        ...
        self.inner
            .send_dialog_response(dialog_id, button, list_item, text.unwrap_or(b""));
    }
    ...
    fn send_exit_vehicle(&self, vehicle_id: u16) {
        self.inner.send_exit_vehicle(vehicle_id);
    }
    fn send_command(&self, text: &[u8]) {
    ...
    #[getter]
    fn get_on_connect(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_connect)
    }
    #[setter]
    fn set_on_connect(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_connect = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_connect = cb.map(|c| wrap0!(c));
    }

    #[getter]
    fn get_on_disconnect(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_disconnect)
    }
    #[setter]
    fn set_on_disconnect(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_disconnect = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_disconnect = cb.map(|c| wrap0!(c));
    }

    #[getter]
    fn get_on_rpc(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_rpc)
    }
    #[setter]
    fn set_on_rpc(&mut self, py: Python, cb: Option<Py<PyAny>>) {
    ...
    #[getter]
    fn get_on_player_join(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_player_join)
    }
    #[setter]
    fn set_on_player_join(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_player_join = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_player_join =
            cb.map(|c| wrap!(c, pid: u16, name: String));
    }

    #[getter]
    fn get_on_player_quit(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_player_quit)
    }
    #[setter]
    fn set_on_player_quit(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_player_quit = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_player_quit =
            cb.map(|c| wrap!(c, pid: u16, reason: u8));
    }

    #[getter]
    fn get_on_chat(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_chat)
    }
    #[setter]
    fn set_on_chat(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        ...
        self.inner.callbacks.lock().unwrap().on_chat = cb.map(|c| {
            Arc::new(move |pid: u16, text: Vec<u8>| {
                with_gil_safe(|py| {
                    let _ = c.call1(py, (pid, PyBytes::new(py, &text)));
                });
    ...
    #[getter]
    fn get_on_client_message(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_client_message)
    }
    #[setter]
    fn set_on_client_message(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        ...
        self.inner.callbacks.lock().unwrap().on_client_message = cb.map(|c| {
            Arc::new(move |color: u32, text: Vec<u8>| {
                with_gil_safe(|py| {
                    let _ = c.call1(py, (color, PyBytes::new(py, &text)));
                });
    ...
    #[getter]
    fn get_on_dialog(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_dialog)
    }
    #[setter]
    fn set_on_dialog(&mut self, py: Python, cb: Option<Py<PyAny>>) {
    ...
    #[getter]
    fn get_on_game_text(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_game_text)
    }
    #[setter]
    fn set_on_game_text(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_game_text = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_game_text =
            cb.map(|c| wrap!(c, s: i32, ms: i32, text: String));
    }

    #[getter]
    fn get_on_set_health(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_set_health)
    }
    #[setter]
    fn set_on_set_health(&mut self, py: Python, cb: Option<Py<PyAny>>) {
    ...
    #[getter]
    fn get_on_set_armour(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_set_armour)
    }
    #[setter]
    fn set_on_set_armour(&mut self, py: Python, cb: Option<Py<PyAny>>) {
    ...
    #[getter]
    fn get_on_set_position(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_set_position)
    }
    #[setter]
    fn set_on_set_position(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_set_position = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_set_position =
            cb.map(|c| wrap!(c, x: f32, y: f32, z: f32));
    }

    #[getter]
    fn get_on_checkpoint(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_checkpoint)
    }
    #[setter]
    fn set_on_checkpoint(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_checkpoint = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_checkpoint =
            cb.map(|c| wrap!(c, x: f32, y: f32, z: f32, sz: f32));
    }

    #[getter]
    fn get_on_checkpoint_disabled(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_checkpoint_disabled)
    }
    #[setter]
    fn set_on_checkpoint_disabled(&mut self, py: Python, cb: Option<Py<PyAny>>) {
    ...
    #[getter]
    fn get_on_player_streamed_in(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_player_streamed_in)
    }
    #[setter]
    fn set_on_player_streamed_in(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        ...
        self.inner.callbacks.lock().unwrap().on_player_streamed_in = cb.map(|c| {
            Arc::new(
                move |pid: u16,
                      ...
                      fs: u8| {
                    with_gil_safe(|py| {
                        let _ = c.call1(py, (pid, team, skin, x, y, z, rot, color, fs));
                    });
                },
    ...
    #[getter]
    fn get_on_player_streamed_out(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_player_streamed_out)
    }
    #[setter]
    fn set_on_player_streamed_out(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_player_streamed_out = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_player_streamed_out =
            cb.map(|c| wrap!(c, pid: u16));
    }

    #[getter]
    fn get_on_player_name(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_player_name)
    }
    #[setter]
    fn set_on_player_name(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_player_name = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_player_name =
            cb.map(|c| wrap!(c, pid: u16, name: String, success: u8));
    }

    #[getter]
    fn get_on_toggle_controllable(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_toggle_controllable)
    }
    #[setter]
    fn set_on_toggle_controllable(&mut self, py: Python, cb: Option<Py<PyAny>>) {
    ...
    #[getter]
    fn get_on_player_time(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_player_time)
    }
    #[setter]
    fn set_on_player_time(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_player_time = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_player_time =
            cb.map(|c| wrap!(c, hour: u8, minute: u8));
    }

    #[getter]
    fn get_on_death_message(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_death_message)
    }
    #[setter]
    fn set_on_death_message(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_death_message = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_death_message =
            cb.map(|c| wrap!(c, killer: u16, player: u16, weapon: u8));
    }

    #[getter]
    fn get_on_set_armed_weapon(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_set_armed_weapon)
    }
    #[setter]
    fn set_on_set_armed_weapon(&mut self, py: Python, cb: Option<Py<PyAny>>) {
    ...
    #[getter]
    fn get_on_spawn_info(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_spawn_info)
    }
    #[setter]
    fn set_on_spawn_info(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        ...
        self.inner.callbacks.lock().unwrap().on_spawn_info = cb.map(|c| {
            Arc::new(
                move |team: u8,
                      ...
                      a3: u32| {
                    with_gil_safe(|py| {
                        let _ = c.call1(py, (team, skin, x, y, z, rot, w1, w2, w3, a1, a2, a3));
                    });
                },
            )
                as Arc<
                    dyn Fn(u8, u32, f32, f32, f32, f32, u32, u32, u32, u32, u32, u32) + Send + Sync,
                >
        });
    }

    #[getter]
    fn get_on_player_team(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_player_team)
    }
    #[setter]
    fn set_on_player_team(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_player_team = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_player_team =
            cb.map(|c| wrap!(c, pid: u16, team: u8));
    }

    #[getter]
    fn get_on_put_in_vehicle(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_put_in_vehicle)
    }
    #[setter]
    fn set_on_put_in_vehicle(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_put_in_vehicle = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_put_in_vehicle =
            cb.map(|c| wrap!(c, vid: u16, seat: u8));
    }

    #[getter]
    fn get_on_remove_from_vehicle(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_remove_from_vehicle)
    }
    #[setter]
    fn set_on_remove_from_vehicle(&mut self, py: Python, cb: Option<Py<PyAny>>) {
    ...
    #[getter]
    fn get_on_player_color(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_player_color)
    }
    #[setter]
    fn set_on_player_color(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_player_color = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_player_color =
            cb.map(|c| wrap!(c, pid: u16, color: u32));
    }

    #[getter]
    fn get_on_world_time(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_world_time)
    }
    #[setter]
    fn set_on_world_time(&mut self, py: Python, cb: Option<Py<PyAny>>) {
    ...
    #[getter]
    fn get_on_toggle_spectating(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_toggle_spectating)
    }
    #[setter]
    fn set_on_toggle_spectating(&mut self, py: Python, cb: Option<Py<PyAny>>) {
    ...
    #[getter]
    fn get_on_wanted_level(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_wanted_level)
    }
    #[setter]
    fn set_on_wanted_level(&mut self, py: Python, cb: Option<Py<PyAny>>) {
    ...
    #[getter]
    fn get_on_weapon_ammo(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_weapon_ammo)
    }
    #[setter]
    fn set_on_weapon_ammo(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_weapon_ammo = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_weapon_ammo =
            cb.map(|c| wrap!(c, wid: u8, ammo: u16));
    }

    #[getter]
    fn get_on_gravity(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_gravity)
    }
    #[setter]
    fn set_on_gravity(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_gravity = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_gravity = cb.map(|c| wrap!(c, g: f32));
    }

    #[getter]
    fn get_on_weather(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_weather)
    }
    #[setter]
    fn set_on_weather(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_weather = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_weather = cb.map(|c| wrap!(c, w: u8));
    }

    #[getter]
    fn get_on_player_skin(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_player_skin)
    }
    #[setter]
    fn set_on_player_skin(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_player_skin = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_player_skin =
            cb.map(|c| wrap!(c, pid: i32, skin: u32));
    }

    #[getter]
    fn get_on_set_interior(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_set_interior)
    }
    #[setter]
    ...
    #[getter]
    fn get_on_vehicle_streamed_in(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_vehicle_streamed_in)
    }
    #[setter]
    fn set_on_vehicle_streamed_in(&mut self, py: Python, cb: Option<Py<PyAny>>) {
    ...
    #[getter]
    fn get_on_vehicle_streamed_out(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_vehicle_streamed_out)
    }
    #[setter]
    fn set_on_vehicle_streamed_out(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_vehicle_streamed_out = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_vehicle_streamed_out =
            cb.map(|c| wrap!(c, vid: u16));
    }

    #[getter]
    fn get_on_player_death(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_player_death)
    }
    #[setter]
    fn set_on_player_death(&mut self, py: Python, cb: Option<Py<PyAny>>) {
    ...
    #[getter]
    fn get_on_textdraw_show(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_textdraw_show)
    }
    #[setter]
    fn set_on_textdraw_show(&mut self, py: Python, cb: Option<Py<PyAny>>) {
    ...
    #[getter]
    fn get_on_textdraw_hide(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_textdraw_hide)
    }
    #[setter]
    fn set_on_textdraw_hide(&mut self, py: Python, cb: Option<Py<PyAny>>) {
    ...
    #[getter]
    fn get_on_textdraw_edit(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_textdraw_edit)
    }
    #[setter]
    fn set_on_textdraw_edit(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_textdraw_edit = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner.callbacks.lock().unwrap().on_textdraw_edit =
            cb.map(|c| wrap!(c, tid: u16, text: String));
    }

    #[getter]
    fn get_on_textdraw_toggle_select(&self, py: Python) -> Option<Py<PyAny>> {
        cloned(py, &self.py_cbs.on_textdraw_toggle_select)
    }
    #[setter]
    fn set_on_textdraw_toggle_select(&mut self, py: Python, cb: Option<Py<PyAny>>) {
        self.py_cbs.on_textdraw_toggle_select = cb.as_ref().map(|c| c.clone_ref(py));
        self.inner
            .callbacks
            .lock()
            .unwrap()
            .on_textdraw_toggle_select = cb.map(|c| wrap!(c, enable: bool, color: u32));
    }
